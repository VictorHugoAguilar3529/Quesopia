...
class AdmonModelo{
  ...
  public function verificarClave($data)
  {
    ...
    //Enviamos el query
    $sql = "SELECT id, password, status, baja FROM administrador WHERE email='".$data['usuario']."'";
    $data = $this->db->query($sql);

    //Verificación
    if (empty($data)) {
      array_push($errores, "No existe el usuario.");
    } else if($clave!=$data['password']){
      ...
      $sql = "UPDATE administrador SET login_dt=NOW() WHERE id=".$data['id'];
