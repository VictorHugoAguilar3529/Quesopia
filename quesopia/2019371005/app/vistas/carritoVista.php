...
for ($i=0; $i < count($datos["data"]); $i++) { 
  ...
  //var_dump($datos);
  //Variables de trabajo
  $desc = "<b>".$datos["data"][$i]["nom_prod"]."</b>";
  $desc.= $datos["data"][$i]["prec_prod"];
  $nom = $datos["data"][$i]["nom_prod"];
  $num = $datos["data"][$i]["id"];
  $can = $datos["data"][$i]["cantidad"];
  $pre =$datos["data"][$i]["prec_prod"];
...
if($verifica){
  print "<td><a href='".RUTA."carrito/gracias' class='btn btn-success' role='button'>Pagar comprando</a></td>";
} else {
  print "<td><a href='".RUTA."carrito/checkout' class='btn btn-success' role='button'>Pagar comprando</a></td>";
}
print "</tr>";
print "</table>";
print "</form>";
